...
public interface ExtraworkProgressRepository extends JpaRepository<ExtraworkProgress, Long> {
	...
	Optional<ExtraworkProgress> findById(Long id);
        ...
        // bring ExtraworkProgressDTO by student & homework
	@Query("SELECT new hyung.jin.seo.jae.dto.ExtraworkProgressDTO(h.id, h.registerDate, h.percentage, h.student.id, h.extrawork.id) FROM ExtraworkProgress h WHERE (h.student.id = ?1) AND (h.homework.id = ?2) ")
	ExtraworkProgressDTO getExtraworkProgress(Long studentId, Long extraworkId);

        // bring ExtraworkProgress by student & homework
